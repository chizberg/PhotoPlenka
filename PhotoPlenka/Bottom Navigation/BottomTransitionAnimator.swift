...
final class BottomTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    private var isPush: Bool { operation == .push }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        to.frame = CGRect(origin: CGPoint(x: newToOriginX, y: to.frame.origin.y), size: to.frame.size)
        ...
        let animations = { [unowned self] in
            ...
            UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 1) {
                ...
                from.frame = CGRect(origin: CGPoint(x: newFromOriginX, y: from.frame.origin.y), size: from.frame.size)
                if !self.isPush {from.alpha = 0}
            }
        }
