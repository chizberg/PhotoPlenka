...
extension NavigationAnimation {
    ...
    private func pushAnimation(from: UIView, to: UIView, container: UIView) -> (() -> ()) {
        ...
        case .sideToSide:
            let (presentationFrame, leftHiddenFrame, rightHiddenFrame) = sideToSideFrames(from: from, container: container)
            container.addSubview(to)
    ...
    private func popAnimation(from: UIView, to: UIView, container: UIView) -> (() -> ()) {
        switch self {
        case .overlay:
            let (presentationFrame, hiddenFrame, _) = overlayFrames(from: from, container: container)
