...
extension BottomSheetPresentationController {
  ...
  private func popDetailsIfNeeded(newHeight: Double) {
    // works only with UINavigationController in bottom sheet
    guard let navigation = presentedViewController as? BottomNavigationController
    else { return }
