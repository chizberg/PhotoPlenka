...
extension BottomSheetPresentationController {
  ...
  func swipeGesture(_ gesture: UIPanGestureRecognizer) {
    ...
    if gesture.state == .began {
      ...
      containerView?.frame = .init(x: 0, y: initialFrame.minY + transition,
                                   width: initialFrame.width,
                                   height: initialFrame.height - transition)

    } else if gesture.state == .ended {
      
      let progressLocation = 1 - (containerFrame.minY / presentingViewController.view.bounds.height)
      UIView.animate(withDuration: 0.15, animations: {
        if progressLocation <= Consts.collapsedThreshold {
          ...
          self.containerView?.frame = .init(x: 0,
                                            y: targetContainerViewY,
                                            width: presentingViewBounds.width,
                                            height: targetContainerViewHeight)
        } else {
          let presentingViewBounds = self.presentingViewController.view.bounds
          let targetContainerViewHeight = presentingViewBounds.height / Consts.openedDelimiter
          let targetContainerViewY = presentingViewBounds.height - targetContainerViewHeight
