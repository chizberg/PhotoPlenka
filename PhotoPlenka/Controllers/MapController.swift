...
extension MapController {
  func configureButtons() {
    ...
    NSLayoutConstraint.activate([
      ...
      favouritesButton.leadingAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.leadingAnchor,
        constant: Constants.sideInset
      ),
      favouritesButton.topAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.topAnchor,
        constant: Constants.sideInset
      ),
    ])
...
extension MapController: BottomSheetHeightObserver {
  ...
  private func cancelAllUpdateRequests(for duration: TimeInterval) {
    ...
    timer = Timer.scheduledTimer(
      withTimeInterval: Constants.mapDelay / 2,
      repeats: true,
...
extension MapController: NavigationControllerObserver {
  func didPush(vc _: UIViewController) {}
  func didPop(newLast _: UIViewController) {}
...
extension MapController: YearSelectorDelegate {
  func rangeDidChange(newRange: ClosedRange<Int>) {
    ...
    self.perform(
      #selector(clearAndLoadNewAnnotations),
      with: nil,
      afterDelay: Constants.mapDelay
    )
  }
}
